...
VERSION=__VERSION__
PRODUCT=__PRODUCT__
BINS="$(echo __BINS__ | tr ':' ' ')"
PRODUCT_HOME="/Library/${PRODUCT}/${VERSION}"

echo "Application uninstalling process started"
# remove link to shorcut file
for exe in ${BINS}; do
  rm -fv "/usr/local/bin/${exe}"
done
if [ $? -eq 0 ]
then
...
#forget from pkgutil
pkgutil --forget "${IDENTIFIER}.$VERSION" > /dev/null 2>&1
if [ $? -eq 0 ]
then
